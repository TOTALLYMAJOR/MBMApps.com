  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-6 py-16 lg:px-8">
      ...
      <div className="mt-10 space-y-5">
        {insights.map((post) => (
          <article key={post.slug} className="surface-panel p-6">
            <p className="text-xs uppercase tracking-[0.2em] text-white/55">{new Date(post.frontmatter.publishedAt).toLocaleDateString()}</p>
            <h2 className="mt-2 font-display text-2xl text-white">{post.frontmatter.title}</h2>
            <p className="text-mbm-muted mt-3 text-sm">{post.frontmatter.summary}</p>
            <Link href={`/insights/${post.slug}`} className="link-theme mt-4 inline-flex text-sm font-semibold">
              Read article
            </Link>
          </article>
        ))}
